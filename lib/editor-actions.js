(function (global) {
  ...
  const ACTION_TYPES = {
    ...
    // Phase 1 — custom page actions
    DELETE_PAGE:      "deletePage",
    // Combined: removes matching page + non-protected section in one shot.
    // Existence of either target is optional — safe to use when you're not
    // sure whether something lives in pages, homeSections, or both.
  ...
  function applyDeleteSection(siteData, action) {
    ...
    // Prune any custom page whose slug matches the deleted section ID so that
    // renderPreviewNavbar doesn't keep a stale page link in the nav.
    if (Array.isArray(siteData.pages)) {
      siteData.pages = siteData.pages.filter((p) => p.slug !== sectionId);
    }
  }

  function validateDeletePage(action, siteData) {
    if (!action.slug || typeof action.slug !== "string") {
      return "deletePage requires a non-empty slug string";
    }
    if (action.slug === "home") {
      return 'Cannot delete the "home" page';
    }
    const exists = (siteData.pages || []).some((p) => p.slug === action.slug);
    if (!exists) {
      return `Page "${action.slug}" not found in pages`;
    }
    return null;
  }

  function applyDeletePage(siteData, action) {
    siteData.pages = (siteData.pages || []).filter((p) => p.slug !== action.slug);
  }

  // deleteContent: removes a matching custom page AND a matching non-protected
  // home section in one shot. Either or both targets may be absent — the action
  // is always a valid no-op when the slug does not exist anywhere.
  // Sets action._nothingFound = true (internal flag) when nothing was removed so
  // the UI can show "Nothing found to delete." instead of "Website updated."
  function validateDeleteContent(action) {
    if (!action.slug || typeof action.slug !== "string") {
      return "deleteContent requires a non-empty slug string";
    }
    if (action.slug === "home") {
      return 'Cannot delete the "home" page';
    }
  ...
  function applyDeleteContent(siteData, action) {
    const pagesBefore    = (siteData.pages        || []).length;
    const sectionsBefore = (siteData.homeSections || []).length;

    siteData.pages = (siteData.pages || []).filter(
      (p) => p.slug !== action.slug
    );
    // Keep a section only if its id does NOT match OR it is protected.
    siteData.homeSections = (siteData.homeSections || []).filter(
      (s) => s.id !== action.slug || PROTECTED_SECTIONS.has(s.id)
    );

    const changed =
      siteData.pages.length        !== pagesBefore ||
      siteData.homeSections.length !== sectionsBefore;
    if (!changed) action._nothingFound = true;
  }
  ...
  const ACTION_REGISTRY = {
    ...
    // Phase 1 — custom page actions
    [ACTION_TYPES.DELETE_PAGE]:      { validate: validateDeletePage,             apply: applyDeletePage },
    [ACTION_TYPES.DELETE_CONTENT]:   { validate: validateDeleteContent,          apply: applyDeleteContent },
  };
